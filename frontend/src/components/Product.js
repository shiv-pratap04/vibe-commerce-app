import React from 'react'
import { useDispatch, useSelector } from 'react-redux';
// Import your new thunks
import { addProductToCart, removeProductFromCart } from '../redux/slices/CartSlice';

const Product = ({post}) => {
    const {cart} = useSelector((state) => state.cart); // Correct
    const dispatch = useDispatch();

    const addToCart = () => {
        // Pass the entire 'post' object (which is your product)
        dispatch(addProductToCart(post)); 
    }
    const removeFromCart = () => {
        // Pass the product's ID
        dispatch(removeProductFromCart(post.id)); 
    }

  return (
    <div className='flex flex-col items-center justify-between hover:scale-110 transition duration-300 ease-in
    gap-3 p-4 mt-10 rounded-xl hover:shadow-[rgba(0,_0,_0,_0.4)_0px_30px_90px]'>
        
        {/* THIS IS THE PART THAT WAS MISSING: */}
        <div>
            <p className='text-gray-700 font-semibold text-lg text-left truncate w-40 mt-1'>{post.title}</p>
        </div>
        <div>
            <p className='w-40 text-gray-400 font-normal text-[10px] text-left'>{post.description.split(" ").slice(0,10).join(" ") + "..."}</p>
        </div>
        <div className='h-[180px]'>
            <img className='h-full w-full'  src={post.image} alt={post.title} />
        </div>
        {/* END OF MISSING PART */}


        <div className='flex justify-between gap-12 items-center w-full mt-5'>
            <div>
                <p className='text-green-600 font-bold'>${post.price}</p>
            </div>

            <div>
                {
                    // Update check: check if a cart item with this 'productId' exists
                    cart.some((p) => p.productId === post.id) ? 
                    (<button onClick={removeFromCart} className='text-gray-700 border-2 border-gray-700 rounded-full font-semibold
                        text-[12px] p-1 px-3 uppercase hover:bg-gray-700 hover:text-white transition duration-300 ease-in '>
                        Remove Item
                    </button>) :
                    (<button onClick={addToCart} className='text-gray-700 border-2 border-gray-700 rounded-full font-semibold
                        text-[12px] p-1 px-3 uppercase hover:bg-gray-700 hover:text-white transition duration-300 ease-in '>
                        Add to cart
                    </button>)
                }
            </div>
        </div>
    </div>
  )
}

export default Product;